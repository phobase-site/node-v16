...
const Home = () => {
  ...
  let viewRowResolver = new RemoteViewRowResolver(getUrqlClient("https://www.phobase.com/api/view/query"));
  ...
  createEffect(async () => {
    ...
    if (view) {
      view.viewId;
      let {data} = await viewRowResolver.readViewRows({viewId: view.viewId});
    ...
    <div class="antialiased text-gray-800 dark:bg-black dark:text-gray-400">
      <main class="container px-8 py-5 lg:py-8 mx-auto xl:px-5 max-w-screen-lg">
        ...
        <div class="grid gap-10 mt-10 lg:gap-10 md:grid-cols-2 xl:grid-cols-3 ">
          <For each={getStandardPosts()}>{post => <StandardPost post={post}/>}</For>
        </div>
      </main>
    </div>
  );
